import { Button, Container, Row, Form, Table, InputGroup } from "react-bootstrap";
import { urlBase } from "../utilitarios/definicoes";

export default function TabelaFuncoes(props) {
        ...
        fetch(urlBase + "/funcao", { method: "GET" })
            ...
            .then((dados) => {
                if (Array.isArray(dados)) {
                    const resultadoBusca = dados.filter((funcao) =>
                        funcao.nome.toLowerCase().includes(termoBusca.toLowerCase()));
                    props.setListaFuncoes(resultadoBusca);
                }
            })
    }
    return (
        <Container className="m -3">
            ...
            <Table striped bordered hover>
                ...
                <tbody>
                    {
                        props.listaFuncoes?.map((funcao) => {
                            return <tr key={funcao.nome}>
                                <td>{funcao.nome}</td>
                                <td>
                                    <Button onClick={() => { props.editarFuncao(funcao) }}>
